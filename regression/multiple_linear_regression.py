...
# Question 01: Making a single prediction (for example the profit of a startup with R&D Spend = 160000,
...
# Important note 2: Notice also that the "California" state was not input as a string in the last column but as
# "1, 0, 0" in the first three columns. That's because of course the predict method expects the one-hot-encoded
# values of the state, and as we see in the second row of the matrix of features X, "California" was encoded as
# "1, 0, 0". And be careful to include these values in the first three columns, not the last three ones, because
# the dummy variables are always created in the first columns.

# ----------------------------------------------------------------------------------------------------------------------

# Question 02: Getting the final linear regression equation with the values of the coefficients
